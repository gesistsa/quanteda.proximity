...
```{r processing}
toks <- tokens(song,  remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE, split_hyphens = TRUE) %>% tokens_tolower() %>% tokens_proximity("fire", count_from = 0)
```

```{r df}
toks_df <- convert(toks)
toks_df[toks_df$proximity <= 15 & toks_df$proximity > 0,]
```
